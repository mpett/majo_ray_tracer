...
class xy_rect : public hittable {
	...
	// using x0 as _x0, x1 as _x1 y0 as _y0 etc

	virtual bool hit(const ray& r, double t_min, double t_max, hit_record& rec) const override;	// Define aarect hit function
    ...
    public:
	shared_ptr<material> mp;	// Enable to use our defined shared pointer mp
...
bool xy_rect::hit(const ray& r, double t_min, double t_max, hit_record& rec) const {
    ...
    if (t < t_min || t > t_max) {       // Checking the boundary
	return false;
    }
    ...
    rec.u = (x-x0)/(x1-x0);         // How far x have gone from x0 on the whole distance x1-x0
    rec.v = (y-y0)/(y1-y0);
    rec.t = t;
